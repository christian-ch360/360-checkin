    ...
    <section className="mx-auto max-w-5xl px-4 py-12 sm:px-8 sm:py-16">
      ...
      {tree.length === 0 ? (
        ...
        <div className="mt-8 space-y-3">
          {tree.map((country) => (
            <details key={country.country} className="rounded-2xl border border-border" open={tree.length <= 2}>
              ...
              <div className="space-y-1.5 border-t border-border/60 px-4 py-3">
                {country.states.map((state) => (
                  <details key={state.state} className="rounded-lg">
                    <summary className="flex cursor-pointer items-center justify-between gap-3 rounded-md px-2 py-1.5 text-sm hover:bg-muted">
                      <Link
