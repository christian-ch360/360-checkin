...
/**
 * The one reusable premium creator card for the CH360 Creator Network — a
 * large photo-forward tile with editorial identity block, category chips,
 * per-platform reach + TOTAL REACH, and the CH360 status marker. `renderAdminControls`
 * adds a subtle management bar for the Operations grid without changing the
 * public look.
 */
export function CreatorCard({
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-black/5">
      ...
      <div className="flex flex-1 flex-col gap-3.5 p-4">
        <div>
          <h3 className="flex items-center gap-1.5 text-[0.95rem] font-semibold leading-tight">
            ...
            {creator.c360Verified ? (
              <BadgeCheck className="size-4 shrink-0 text-[var(--community)]" aria-label="Verified CH360 member" />
            ) : null}
          </h3>
        ...
        <div className="flex items-center justify-between gap-2 pt-0.5">
          ...
          <Link href={href} className="text-xs font-medium text-[var(--community)] hover:underline">
            View Profile →
          </Link>
        </div>

        {renderAdminControls ? (
          <div className="-mx-4 -mb-4 mt-1 border-t border-border bg-muted/40 px-4 py-2.5">{renderAdminControls}</div>
        ) : null}
      </div>
    </article>
  );
